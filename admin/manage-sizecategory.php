                            ...
                            <div class="card-body tab-content">
                                <div class="tab-pane <?php if(empty($_GET['q'])){ echo 'active';}?>" id="first2">
                                    <div class="table-responsive">
                                        ...
                                        <tbody>
                                        <?php $i=1;
                                        $sql=mysqli_query($GFH_Admin::$link,"SELECT * FROM `size_category`");
                                       while( $na=mysqli_fetch_array($sql)){?>
                                        <tr class="gradeX">
                                            <td><?php echo $i;?></td>
                                            <td><?php echo $na['title'];?></td>
                                             <td><?php if($na['status']==1) { echo 'active'; }else{ echo "inactive";}?></td>
                                ...
                                <div class="tab-pane <?php if(!empty($_GET['q'])){ echo 'active';}?>" id="second2">
                                    <form method="post" class="form form-validate floating-label" enctype="multipart/form-data">
                                        <?php if(!empty($_GET['delete'])){
                                          $sql= mysqli_query($GFH_Admin::$link,"DELETE FROM `size_category` WHERE `id`='".$_GET['delete']."'");
                                        }


                                        if(!empty($_GET['q'])){
                                        $sql=mysqli_query($GFH_Admin::$link,"SELECT * FROM `size_category` WHERE `id`='".$_GET['q']."'");
                                        $naa=mysqli_fetch_array($sql);

                                         if(isset($_POST['title'])){
                                         $sql= mysqli_query($GFH_Admin::$link," UPDATE `size_category` SET `title`='".$_POST['title']."',`status`='".$_POST['status']."' WHERE `id`='".$_GET['q']."'");
                                         if($sql){
                                ...
                                $sql= mysqli_query($GFH_Admin::$link,"INSERT INTO `size_category` SET `title`='".$_POST['title']."',`status`='".$_POST['status']."'") or die('erro');
                                    ...
                                    ?>
                                       ...
                                       <div class="col-md-7">
                                           <div class="form-group">
                                               ...
                                               <label for="Email1">Size Category Title</label>
                                           </div> 
                                           </div>
                                            <div class="col-md-5">
                                            <div class="form-group">
                                            <select id="select1" name="status" class="form-control" required="" aria-required="true">
                                                       <option value=""></option>
